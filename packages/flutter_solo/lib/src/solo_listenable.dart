...
///     with SoloListenable {
///   ProfileController() : super(const Empty());
/// }
/// ```
///
...
/// [onListenerError], and which report wins follows the chain. The class
/// that mixes `SoloListenable` in, and every class below it, overrides the
/// mixin: a base that mixes it in and overrides [onListenerError] keeps its
/// own report. The classes it is mixed onto are overridden by it: a base
/// without Flutter, with the mixin on the leaf, loses its override to this
/// one, and the leaf cannot reach it through `super` either, which lands
/// here. A base that wants its own report keeps it in a method of another
/// name, and the leaf's override calls that.
///
