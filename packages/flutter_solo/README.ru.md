...
подписывалась бы и отписывалась каждый кадр. Значения сравниваются через
`==`, если не передан `equals:`; на источник проекция подписана, только
пока у неё есть слушатели, и освобождать её не нужно. `value` читает
состояние каждый раз, поэтому никогда не отстаёт — держите селектор
дешёвой выборкой.
