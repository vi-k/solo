...
| Вопрос | Ответ |
| --- | --- |
| Когда зовут слушателей? | Синхронно, в порядке подписки, на каждое изменение состояния. `stream` у `Solo` продолжает работать и приходит микротаской позже; `value` и `currentState` — один и тот же объект. |
