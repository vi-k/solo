...
Длинный разбор, одиннадцать сценариев, решённых сначала на bloc, а потом
здесь, —
...
дожидается своего вызова в любом случае — сохранение не рвётся пополам.
`onError` и `onCancel` — дорога назад: они говорят, в каком состоянии остаётся
упавшая или отменённая загрузка, а без них экран держал бы спиннер задачи,
которой уже нет. Всё API целиком — правила, очередь, дети, наблюдатели —
...
участник отказывается отпускать, остальные всё равно отменяются, а каждый отказ
...
если контроллер не переопределяет `onUnanswered` и за него не отвечает
...
broadcast-стрима `SoloStream`, который виджету ни к чему. Если вы пишете
не виджеты, берите его — он на чистом Dart.
