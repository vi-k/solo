...
[jobs](https://pub.dev/packages/jobs).
...
пакете [jobs](https://pub.dev/packages/jobs), а `solo` добавляет поверх
состояние, очередь и правила. Пакет реэкспортирует его целиком, поэтому
`package:solo/solo.dart` — единственный нужный импорт. Вместе с ним приходят
и его типы, они видны в автодополнении: `JobObserver` может вам
пригодиться, а `JobContext`, `JobBase`, `JobContextBase`, `DeferredJob` и
`JobStatus` — для собственного движка, и объясняются они в README `jobs`, а
не здесь.
...
отмечена, а `log`, `job` и три регистрирующих уборку (`onDispose`,
`onDiscard`, `disown`) работают по-прежнему. Поэтому тело не делает `await` само по себе: каждый
...
не задано ни то ни другое, слушать некому, и ошибка уходит в зону, в
которой построен контроллер, — так же, как ядро отдаёт ошибку, которой
больше некуда идти; уборщик, тронувший контекст и остановившийся на первой
строке, будет услышан, а не потерян. Поставьте наблюдателя на старте, как
сказано в разделе [Ошибки](#ошибки), и маршрут станет вашим.
...
перед уборкой, — и не завершается вовсе у задачи, кончившейся `Done` или
`Failed`; `job.cancel()` отменяет и
ждёт, пока задача действительно завершится; `job.ignore()` говорит, что на исход никто смотреть не будет.
