...
контроллера в теле видны. Обычное чтение, названное `state`, выглядело
бы в точности как `ctx.state`, и тело, набравшее его по привычке, читало
бы состояние после отмены, которую обязано было соблюсти: ни `Cancelled`,
...
содержит соответствия API и сравнивает десять прикладных сценариев
