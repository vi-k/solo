...
`SoloContext`, кроме `log` и `job`, бросает `Cancelled` этой задачи, как
только она отмечена. Поэтому тело не делает `await` само по себе: каждый
...
через сам вызов. `each` — это
extension на `JobContext` (интерфейсе ядра, который реализует
`SoloContext`), а не его член: он собран из `wait` и `onCancel` и не делает
ничего, чего не могло бы ваше собственное тело.
...
отменившего себя само, только вместе с концом задачи — и не завершается
вовсе у задачи, кончившейся `Done` или `Failed`; `job.cancel()` отменяет и
