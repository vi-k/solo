...
дальше; в переоценке после смены состояния ошибка уходит в `onError`, а
задача работает дальше, потому что бросившее правило ничего не сказало о
том, можно ли ей; при следующем чтении состояния ошибка летит в тело, как
любая другая. Каждый из этих путей сперва приходит в `onError`, а оттуда в
зону, если никто не смотрит. Правила — ваш обычный код: бросать они не
должны, и движок не делает вид, будто не могут.
