...
`discard` у вызова закрывает значение, которое до тела не доехало: отмена
бросает внутри вызова контекста, и `join` иначе его выбросил бы, а у
`wait` его и не было. `onDispose` закрывает весь остаток задачи: ошибку,
отмену между шагами, отмену, пришедшую после `return`. Для значения,
которое тело возвращает или передаёт наружу, берут `onDiscard`, для всего
остального — `dispose`/`onDispose`: `discard` на ресурсе, который остаётся
внутри тела, на успешном пути не сработает, и эта утечка — единственная
ошибка, которую движок поймать не может.
