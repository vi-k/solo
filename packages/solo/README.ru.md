...
берёт десять сценариев из разных предметных областей, решает каждый сначала
...
`profile.state` синхронно возвращает текущее состояние;
`profile.stream` — broadcast-стрим изменений, доставляемых микротаской
позже. Корневые Job выполняются по одной. Следующая запускается после
завершения тела, детей, уборки и итогового обработчика состояния
предыдущей Job.

`close()` прекращает приём работы, отменяет Job в очереди, запрашивает
отмену выполняющейся Job и ждёт её завершения. Job, добавленные после
закрытия, сразу завершаются с `Cancelled(closed)`.
...
`ctx.uncancellable(() => ...)` откладывает отмену до завершения действия;
бросает её следующая контрольная точка отмены. Обычный код после вызова
может выполниться. `ctx.unattended(() => ...)` не ждёт вовсе: отдаёт
...
`return`. В обратном — пока держится исход: `discard`, пропущенный потому,
что тело вернуло значение, движок подберёт вторым проходом, если отмена
придёт по ходу уборки, — и тогда тот отработает после уборщиков,
зарегистрированных ниже него.
...
Устройство может потерять связь независимо от контроллера. Его слушатель
сообщает об этом через защищённый метод `externalSetState`, устанавливая
`Disconnected`. Это состояние сохраняется, в том числе если уведомление
пришло во время уборки Job, уже отменённой вручную. Совместимое внешнее
состояние разрешает коррекцию, поэтому повторные проверки типа не нужны.
...
**Слушание стрима.** `ctx.each` запускает и возвращает дочерний `Job<void>`,
которому принадлежит подписка. Колбэк получает `SoloContext<S, W>` этого
ребёнка, поэтому чтения состояния, записи и проверки отмены принадлежат
ребёнку:
...
Пример возвращает `.value` ребёнка, чтобы ошибка потока или колбэка
дошла до тела родителя; отмена бросается как `Cancelled`. Чтобы прочитать
исход, ожидайте `.done`. Чтобы отдельно остановить эту подписку, сохраните
возвращённый `Job` в `subscription` и ожидайте его `cancel()`. Отмена ребёнка
сама не помечает родителя отменённым, но неперехваченный `Cancelled`
из `.value` отменяет родителя по обычным правилам дочернего `Job`.

Родитель ждёт этого ребёнка, даже если его тело вернулось, не ожидая его.
Поэтому открытый поток удерживает родителя, даже когда событий нет.
Принятая отмена родителя передаётся ребёнку, в том числе при `close()`.
Ребёнок отменяем, даже если родитель неотменяем. Он сохраняет рабочий тип
`W` и `keepWhile` родителя, поэтому правила защищают доступ к состоянию
и после возврата тела родителя. `canStart` не наследуется: это условие
уже разрешило запуск родителя. Наблюдатели видят ребёнка отдельно.

События сохраняют порядок: асинхронный колбэк ожидается перед доставкой
следующего события, а первая ошибка потока или колбэка останавливает
обработку. Приняв отмену, ребёнок сразу снимает подписку и прекращает
доставку, затем дожидается текущего колбэка, прежде чем завершиться
и освободить ресурсы. Уборка родителя тоже ждёт. Используйте внутри
колбэка `wait`, `join` и другие контрольные точки дочернего контекста:
голый `await` нельзя прервать, и он может задержать отмену и `close()`
навсегда. Не ждите завершения самого ребёнка или его `cancel()`
из колбэка: ребёнок уже ждёт этот колбэк.

Future, которую возвращает `cancel()` самой подписки, не ожидается.
При необходимости ждите асинхронную уборку источника отдельно. Обычное
завершение потока по-прежнему зависит от отправки `onDone` источником.
`each` — метод контекста с теми же ограничениями жизненного цикла, что
и `run`: он не может запустить ребёнка после конца тела родителя,
из `unattended` или уборки.
...
ребёнка из `await ctx.run(child)`; тело, которое *упало*, оставляет их
доработать и дожидается. `await ctx.run(child)` возвращает значение ребёнка
после завершения его детей и уборки, проверяя отмену и правила состояния
родителя перед продолжением. Ошибка или `Cancelled` ребёнка бросается
в тело родителя со своим стеком. Ребёнок может отклонить отмену; `run`
дождётся его и проверит родителя после успешного результата.

Сохраните исходный `child` для отмены и просмотра `child.done`; `run`
возвращает Future и не создаёт новый Job. Чтобы родитель и ребёнок работали
параллельно, обработайте возвращённую Future отдельно. Тогда их записи
состояния могут чередоваться. Если результат намеренно не нужен,
используйте `ctx.run(child).ignore()`; родитель всё равно дождётся своих
детей перед завершением.

**Цепочки.** `job.then((ctx, value) => ...)` создаёт `Job` ядра, который
запускается после успешного завершения `job`, включая его детей и уборку.
Обработчик получает новый `JobContext` и возвращает значение или future.
Провал передаётся дальше без вызова обработчика. Отмена идёт вперёд
к продолжениям и назад к незавершённым предшественникам; отмена хвоста
ждёт эту работу и уборку. Источники сохраняют свои обычные правила отмены,
включая отказ и задержку.

У продолжения собственный необязательный `observer`. Оно не наследует
`SoloContext`, правила, наблюдатель контроллера или слот очереди. Чтобы
изменить состояние, вызовите метод контроллера, который добавит другую
задачу в очередь; между этими шагами могут пройти другие задачи.
`close()` отменяет продолжение через незавершённый источник, но не владеет
продолжением, уже работающим после завершения источника. Используйте
детей внутри одного родителя, когда вся последовательность должна
удерживать текущий слот контроллера.

**Внешнее состояние.** Устройство или другая независимая сущность может
изменить своё состояние без команды нашего контроллера. Слушатель внутри
наследника `Solo` отражает уже произошедшее изменение через
`externalSetState(next)`, помеченный `@protected`. Это исключение из
изменения состояния через Job: переходом владеет внешняя сущность,
контроллер получает её уведомление. Для собственных операций контроллер
использует тела Job и обработчики состояния `run`/`job`.

Внешнее обновление переоценивает правила выполняющихся тел. Для Job
с обработчиками состояния несовместимое обновление также запрещает
коррекцию при отмене, ожидании детей и уборке.

Остановите внешний слушатель перед закрытием контроллера.
`externalSetState` продолжает менять состояние после `close()`,
но закрытый стрим `Solo` уже не доставляет эти обновления.
...
`HandlerCancelReason` и `ChainCancelReason` из ядра, `RulesCancelReason`
и `ClosedCancelReason`
...
Обработчик получает `Cancelled` с причиной и подробностями: при принятии
отмены работающей задачей, при снятии задачи до старта или после окончания
тела и его детей, перед уборкой, если тело отменило себя само. Поздняя
регистрация вызывает его сразу. Задачи с исходом `Done` или `Failed` без
отмены освобождают обработчики без вызова. Ошибки обработчиков идут путём
`ctx.onCancel`; `async`-колбэки не ожидаются. `job.cancel()` отменяет задачу
и ждёт её фактического завершения; `job.ignore()` говорит, что исход никто
смотреть не собирается.
...
поэтому `load()`, возвращающий `String`, и `refresh()`, возвращающий
`void`, одного ключа делить не должны. Никто этого за вас не проверит —
приведение упадёт во время работы, `TypeError` из `add`. Enum ключей, как в
`example/`, — способ развести их руками: по константе на метод, и правило в
том, что у типа результата свой ключ. `job`, `add` и `run` публичны потому,
что их зовут собственные методы контроллера; API, которым пользуется
вызывающий, — это его методы.

**Аккумуляция событий.** Создайте накопитель один раз через `collect`,
чтобы хранить события списком, или через `accumulate`, чтобы сливать их
в одно значение. Его метод `add(event)` собирает вход для ожидающего
`SoloJob`; обработчик меняет состояние, когда эта задача выполняется.
`AccumulationPolicy.adjacent` присоединяет только к хвосту очереди,
`replace` переносит вход в новую задачу в хвосте и отменяет прежнюю,
а `join` дополняет существующую ожидающую группу на её месте. Объединяются
только группы одного накопителя; работающие группы новые события
не принимают. Примеры, порядок и исходы хэндлов разобраны в
[рецепте настроек и логов](../../docs/ru/solo/accumulation.md).

Параметр `timing: AccumulationTiming.debounce(duration)` задаёт ожидание
паузы между событиями, а `AccumulationTiming.throttle(duration)`
ограничивает частоту стартов групп. `collect` сохраняет каждое принятое
событие, `accumulate` — значение, выбранное `merge`. Ожидающие группы
остаются в `queue` и пропускают готовые Job. Обработчики выполняются
последовательно. Без `timing` или при `Duration.zero` группа готова сразу.
...
`ctx.run(child)` возвращает `Future<T>`. Дождитесь её, чтобы передать ошибку
или отмену ребёнка в тело. Чтобы вместо этого посмотреть исход, сохраните
исходный child, вызовите `ctx.run(child).ignore()` и дождитесь `child.done`.
Родитель ждёт детей, даже если тело их не ожидает, но возвращённая Future
всё равно требует обработки ошибок: одного `child.ignore()` недостаточно
для обработки ошибок этой Future. Необработанная ошибка Future, включая
отмену, подчиняется обычным правилам Dart.
...
`onError` конечная, когда он есть: контроллер, который его переопределил,
или `SoloObserver` в `SoloBase.observer`. Если нет ни того ни другого, та
же ошибка уходит в зону, где контроллер создали, а в корневой зоне это
процесс. Правила — ваш обычный код: бросать они
...
`Cancelled` в зону не уходит никогда. Ошибка, пришедшая после отмены
задачи, — действие, которого `wait` перестал ждать и которое упало
позже, — доходит до `onError` и там останавливается, **если есть кому
слушать**: без наблюдателя и без переопределённого хука она идёт в зону
той же дорогой, что любая ошибка, которой некуда деться.
...
**Debounce.** Накопитель позволяет запускать поиск после паузы во вводе.
Здесь `merge` сохраняет последний запрос, а debounce ждёт 300 мс тишины.
За время паузы могут выполниться другие готовые Job. Уже работающий поиск
завершается перед стартом следующего. Возвращённый Job позволяет получить
исход и запросить отмену; обработка сбоев описана в [Ошибках](#ошибки).
...
Рецепт просит от вас двух вещей. Стрим несёт то, что случится дальше, а не
то, что уже случилось, поэтому задача сначала берёт состояние, за которым
следит, — `session.state`, — и только потом слушает; без этой первой
строки экран следит за сессией, которую ни разу не прочитал. И задача
держит очередь всё время, пока следит, поэтому место ей в контроллере,
чья работа — слежение: контроллеру со своими задачами нужна задача на
событие, поставленная из колбэка.
...
[`example/`](example) — работающий пакет с этим
