...
`ctx.uncancellable(() => ...)` отказывает отмене на время вызова. Там, где
...
`ctx.uncancellable(action)` говорит то же самое про один шаг тела, а не про
всю задачу, и это третий ответ, который может получить вызов: `wait`
прекращает ожидание, но не работу, `join` дожидается работы и сдаётся после
неё, а этот на время `action` вовсе не даёт себя отменить. Годится для шага,
который назад не отыграть, — платёж, уже ушедший на сервер, запись, уже
идущая по проводу. Пришедшие в это время `cancel` и `close` получают отказ,
а не откладываются, `close` дожидается тела, и правила этим тоже не покрыты.
Участки вкладываются друг в друга, а ответ, действовавший до шага,
возвращается после — бросил шаг или нет; поэтому задача, созданная с
`cancellable: false`, такой и остаётся до конца, а шаг, который внутри неё
обязан быть прерываемым, стоит вынести в отдельную задачу.
