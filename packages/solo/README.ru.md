...
`cancellable` — это один и тот же вопрос, заданный дважды. При создании
это ответ за всю задачу; внутри тела `ctx.cancellable` — ответ на сейчас, а
создание лишь задаёт, с чего он начинается. Закройте его вокруг шага,
который назад не отыграть, — платёж, уже ушедший на сервер, запись, уже
идущая по проводу, — и откройте обратно. Пока закрыто, `cancel` и `close`
получают отказ, а не откладываются, и `close` дожидается тела; правила этим
тоже не покрыты. Возвращайте в `finally`, если шаг может бросить: это
единственный член контекста, который сам не бросает никогда.
