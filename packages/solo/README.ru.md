...
стрим, а выводит из этого тело отмена задачи. `ctx.state` внутри колбэка —
обычное чтение, проверяемое по правилам; `Cancelled`, который оно может
бросить, — не ошибка, а конец стрима, и возвращается он через сам вызов.
Колбэк, вернувший футуру, дожидается, и доставка на это время придержана:
события сохраняют порядок, а колбэк, который бросил, не зовут снова.
Придержана доставка, а не задача: колбэк, уже ушедший в своё ожидание,
отмена не прерывает, и не ждёт его никто — ни вызов, ни уборка, которая
может закрыть под ним то, во что он ещё пишет. `each` — это extension на
`JobContext` (интерфейсе ядра, который реализует `SoloContext`), а не его
член: он собран из `wait`, `onCancel`, `onDispose` и `job` и не делает
...
final class ScreenController extends Solo<Screen> {
  ...
  Job<void> follow() => run<Screen, void>(
        ...
        (ctx) async {
          ...
          take(session.state); // то, что уже случилось
          await ctx.each(session.stream, take); // и то, что случится дальше
        },
      );
}
```
