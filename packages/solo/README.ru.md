...
**Взятие во владение.** `job(...)` и `run(...)` тоже принимают
`ifCancelled` — для значения, которое тело вернуло, когда отмена уже
пришла: между `return` и исходом задача ещё жива (она ждёт детей), и
пришедшая туда отмена побеждает. Исходом всё равно будет отмена, а
значение уходит диспозеру, а не на пол, и `close()` его дожидается.

Отмена бросает внутри вызова контекста, поэтому
значение, которое вызов собирался отдать, до тела не доходит: `join` его
выбрасывает, а у `wait` его и не было. Оба принимают `ifCancelled` ровно
для этого значения — соединения, файла, подписки, которые иначе утекут.
`join` дожидается освобождения до броска, поэтому его дожидается и
`close()`, и следующая задача стартует, когда ресурса уже нет; `wait`
зовёт его поздно, когда брошенное действие наконец вернётся, и этого не
ждёт никто. А когда значение уже в руках у тела, дальше работает обычный
`finally` — с голым `await`, потому что отменённой задаче откажет любой
член контекста, в том числе на уборке:
...
Job<void> load() => run<Idle, void>(
      ...
      (ctx) async {
        ...
        try {
          ...
          // Задача к этому моменту может быть отменена, поэтому этот один
          // вызов идёт без контекста; `close()` всё равно ждёт тело.
          await db.close();
        }
      },
    );
