...
удаляющих метода пропускают задачи с `Cancellable.never`, если не передан
...
`cancellable` говорит, кто ещё может остановить задачу и когда.
`Cancellable.always`, по умолчанию, — кто угодно и когда угодно: ручной
`cancel`, `clear`, отмена родителя, `close`. `Cancellable.whileQueued`
позволяет очереди снять задачу, пока та ждёт, и защищает тело, как только
оно началось: платёж, уже ушедший на сервер, запись, уже идущая по проводу.
`Cancellable.never` защищает и в очереди, и там путь мимо нёе — `force:
true`, предназначенный контроллеру, который сносит сам себя, а не обычному
решению.

Ни один из трёх не защищает задачу от реальности: если состояние перестало
подходить по `W` или `keepWhile`, задача отменяется всё равно, потому что
читать ей уже нечего. Задаче, которая должна пережить любое состояние, нужен
базовый тип `S` и никакого `keepWhile`.
...
`Cancellable.never` и никогда не трогает выполняющуюся задачу — сам снимок.
