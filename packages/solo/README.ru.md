...
результата. Тело получает контекст `ctx` для изменения состояния и ожидания
с учётом отмены:
...
состояние после отмены, которую обязано было соблюсти: ни `Cancelled`,
ни проверки `keepWhile`, и весь `S` вместо рабочего типа задачи `W`.
...
Всё, чем пользуются каждый день, в одном контроллере. `PlayerState`, `Api`
и `Device` принадлежат приложению; остальное — пакет.
...
| Поздний запрос не должен быть отброшен как дубликат раннего | ключ-запись, `(Op.load, id)` | [Очередь и политики](../../docs/ru/solo/jobs.md#очередь-и-политики) |
...
| `close()` не возвращается | `Solo.pending` | [Что удерживает контроллер](../../docs/ru/solo/errors.md#что-удерживает-контроллер) |
...
| Шаг нельзя прервать на половине | `ctx.join` для вызова, `ctx.uncancellable` для шага | [Защита шага или всей задачи](../../docs/ru/solo/cancellation.md#защита-шага-или-всей-задачи) |
...
| Очередь нужно на время остановить | задача, ждущая `Completer` в её голове | [Пауза очереди](../../docs/ru/solo/jobs.md#пауза-очереди) |
| Событие стрима приходит микротаской позже, и это поздно | `publish` в наследнике `Solo`, уведомляющий внутри изменения | [Своя доставка](../../docs/ru/solo/state.md#своя-доставка) |
