...
A plain `Future` has no cancellation. A flag can ask work to stop, but
nothing answers for what the work has already opened:
...
through the context, `ctx`, passed to the body. `ctx.join(action)` checks
...
body. `CancelableOperation` cancels the waiting; a job owns what the work
left behind.
...
- **`ctx.uncancellable(() => database.markReady(stop))`** protects this
  ...
  checkpoint throws `Cancelled`. See [Cancellation](doc/cancellation.md).
