> **Состояние на 2026-09-16:** сделано и смержено, коммиты `84ee53f`…`72b648e` (план — `2026-09-16[3]-solo-stream-mixin-plan.md`), плюс правки по кругу ревью, коммиты `fea9ca3` и далее. Спека прошла два независимых ревью до плана, оба открытых вопроса решены владельцем 2026-09-16.
...
`close` — реентерабельный из хука и плоский поверх дренажа — отдаёт ту же
future; сосед по `with`, чей `publish` зовёт `super` и после этого бросает,
глушит стрим, стоя под `SoloStream`, и не глушит, стоя над ним — то есть
именно то, что измерено в разделе «Риски» выше, а не гипотетический сосед,
не зовущий `super` вовсе: такого ловит `@mustCallSuper` до всякого теста.
**Поправка 2026-09-16, круг ревью:** первая редакция этого критерия называла
другой механизм (не зовущий `super` сосед) и противоположную полярность —
исправлено на то, что действительно измерено и проверено тестом.

**Перевод.** Девять объявлений получили миксин, по одному отдельно
проверенному кандидату каждое (шаг 0 плана: пять приватных фикстур
`extends Solo<TestState>`, для каждой явно решено, читает она `.stream`
или нет). **Поправка 2026-09-16, круг ревью:** «отчёт перечисляет все
просмотренные вхождения `extends Solo` по файлам» — невыполнимое
требование при 74+ таких вхождениях без утраты веса; настоящая защита
сильнее ручного перечня — любое из них, которое попробует прочитать
`.stream` без миксина, не компилируется, и это проверяет каждый прогон
`dart analyze`, а не разовый обзор. Критерий сведён к тому, что
действительно даёт гарантию: сборка чиста. Отдельно закрыты два места,
где посылка теста утверждала наличие стрима без него —
`solo_builder_test.dart` (`_StreamController` → `_PlainController`)
и проверка `solo_selection_test.dart` (посылки не было).
