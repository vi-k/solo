> **Состояние на 2026-09-16:** оба шага сделаны и смержены. Шаг 1 —
> код и тесты, коммиты `84ee53f` и `1eaa283`. Шаг 2 — документы,
> коммиты от `a0cdaa8` до `54c460e`. Работа закрыта целиком.
...
7. **Механическое переименование `SoloBase` → `Solo` по всему дереву** —
   ...
   найдено **78 обращений к статическим членам**
   (`SoloBase.observer`/`.errorHandler`/`.debug`, по
   `git grep -o -E 'SoloBase\.(observer|errorHandler|debug)'` в
   `packages/solo/test` и `packages/flutter_solo/test`), плюс пятнадцать
   `extends SoloBase<`/`SoloBase<S>` как тип. Замена везде, где `SoloBase`
   встречается как идентификатор,
...
истиной. Переименована в `_PlainController`, комментарий поправлен. Название
самого теста (`'takes a plain Solo, which no ValueListenableBuilder would'`)
трогать не пришлось — оно уже говорило правду и без стрима.
**Поправка круга ревью:** предыдущая версия этого абзаца утверждала, что
название теста тоже поправлено — неточность, дифф этого не делал.
`solo_selection_test.dart:28` из того же пункта спеки при чтении оказался
без ложной посылки — `_PlainController` там ничего про стрим не утверждает,
и трогать нечего.
...
**Сделано, все проверки зелёные.** `vs-bloc.md` правился первым, как советовал
советник — единственная страница со стендом, и именно там нашлась находка
не из списка спеки: `TelemetryObserver.onChange` брал `SoloBase<Object>` внутри
фрагмента `2/RecorderController`, а не только
`PlayerController`/`ReportController` теряли `.stream`. По той же причине
нашёлся пропуск в `packages/async_job/CHANGELOG.md` — там своё упоминание
`SoloBase.errorHandler` в `Unreleased`, не названное ни спекой, ни этим планом,
поймано финальным сквозным `grep` по всему дереву, а не по списку.
В `CHANGELOG.md` обоих пакетов и `errors.md` добавлен разбор случаев, где
«Solo» в старых записях `Unreleased` означало прежний стримовый класс,
а не движок, — блайндовая замена имени сломала бы им смысл, а не только имя.
Финальный прогон: `dart doc` — 0 warnings, 0 errors, оба пакета; сборка сайта —
