...
свежим прогоном, а не переносятся из спеки или из этого плана (в дереве после
этого шага уже была независимая работа с тех пор, как спека называла
577/388/9/87). Три сторожа из критериев приёмки спеки («Механика»): стрим
закрывается после движка; повторный `close` отдаёт ту же future; сосед
по `with`, не зовущий `super.publish`, глушит стрим сверху и не глушит снизу —
уже есть в наборе `close_test.dart`/`hooks_test.dart` по замерам ревью,
отдельно писать не нужно, только убедиться, что они остались зелёными после
переноса.
