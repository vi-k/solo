> **Состояние на 2026-09-07:** исполнен целиком и смержен в `main`
> (`2e91577`..`cb2e4be`). Вторая редакция, написанная по кругу ревью
...
> приняты. Отличия исполнения от текста плана — в конце записи.
...
Исполнен задача за задачей, коммит на задачу, 2026-09-07. Отличий от
текста немного, и все мелкие:

- **Задача 5.** Тест на строку `debug` встал в `debug_test.dart` не в
  `fakeAsync`-блок целиком: `JobBase.debug` ставится до него, а проверка
  идёт после, иначе `addTearDown` не успевает. Хелпер — `ProbeJob` с его
  `launch()` и `drop()`, как и сказано в плане.
- **Задача 5.** Кроме названных мест, `ifCancelled` упоминался в
  дартдоках `SoloBase.job` и `SoloBase.run` и во внутреннем имени
  параметра `_dispose` — вычищено там же.
- **Задача 8.** Раздел README «Late values» переименован в «Cleanup»:
  поздние значения стали одним случаем стека, а не механизмом.
- **Сверх плана, по вопросу владельца.** В «Quick start» ядра, в примере
  и в README `solo` стояла двойная регистрация одного значения
  (`discard:` у вызова плюс член рядом) — та самая, от которой
  предостерегает дартдок `disown`. Стенд показал два закрытия базы;
  вторая регистрация убрана (`4bdcc23`). Там же, в разделе «Дети»,
  объяснено, почему ребёнок создаётся через `Job.deferred` (`cb2e4be`).

Проверки на момент мержа: `jobs` — 119 тестов, `solo` — 244, пример
`solo` — 7, `flutter_solo` — 8; `dart analyze` без замечаний во всех
пакетах, `dart doc` без предупреждений у обоих, `dart format` без правок,
`python3 tool/check_translations.py` — без расхождений.
