> **Состояние на 2026-09-18:** вторая редакция после круга ревью. Два
> ревьюера Opus, каждый в своей копии дерева, оба — «годится с правками»; все
> находки приняты, две — частично, вердикты в записях ревью. Кода по спеке
> ещё нет.
...
`CHANGELOG`, если перезаливка его даст, разбирается глазами и допустимо только
как позиция типа. В позиции типа аргумент на месте — проверено глазами
по `grep -rnwE "SoloStream|SoloListenable"` в коде и фрагментах: линт этого
не страхует.
