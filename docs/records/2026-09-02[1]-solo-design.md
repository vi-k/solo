> **Состояние на 2026-09-02:** черновик. Секции «Публичный API», «Движок и
> очередь», «Отмена и результат», «Дети и внешнее состояние» подтверждены
> владельцем в диалоге, секция «Тесты и пример» представлена без возражений.
> Ждёт прочтения владельцем целиком. Реализация не начата.
> **Что это:** устройство и публичный API пакета `solo`, преемника
> `conveyor` 1.x.
> **Связанные записи:** пока нет. Следующая — план реализации.
...
- Реентерабельно: листенер стрима может позвать `externalSetState` внутри
  чужого `emit`; отмена идемпотентна.
