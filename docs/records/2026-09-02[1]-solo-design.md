> **Состояние на 2026-09-04:** реализована целиком и смержена в `main` по
...
> 3.1, 4.8, 4.9, 6.2, 7.3). Пересмотрена 2026-09-04: булев `cancellable`
> стал enum `Cancellable` (`always`, `whileQueued`, `never`) — всё, что
> сказано здесь про `cancellable: false` и про роль `force`, читать по
> `2026-09-04[3]-cancellable-design.md`. Тогда же изолированы вызовы хуков,
> см. `2026-09-04[2]-hook-isolation-report.md`.
> **Что это:** устройство и публичный API пакета `solo`, преемника
> `conveyor` 1.x.
> **Связанные записи:** `2026-09-02[2]-solo-plan.md` (план реализации),
> `2026-09-04[3]-cancellable-design.md` (пересмотр `cancellable`),
