> **Состояние на 2026-09-08:** написан, к исполнению не приступали.
> Следующий шаг — независимое ревью плана, как у
> `2026-09-07[6]-dispose-plan-review.md`.
...
> каждую.
> **Связанные записи:** `2026-09-08[4]-detached-design.md` (спека, вторая
...
тела»). Форк несёт zone-value с зоной вызова и с задачей-хозяином: по
первому конструктор `JobBase` берёт зону, куда отчитываться, по второму
`run` и `uncancellable` узнают, что их позвали из фона, и отказывают.
...
7. **Отказ после конца — своей проверкой, не `throwIfFinished`.** Спека в
   ...
   unattended work'`.
8. **Имена внутренних сущностей.** `_UnattendedFork` — запись
   `({Zone from, JobBase<Object?> owner})`, которую несёт форк;
   `_unattendedKey` — ключ zone-value, свежий `Object()`, так что назвать
   его снаружи библиотеки нечем; `throwIfUnattended(String action)` —
   защищённая проверка «меня позвали из фона этой же задачи»;
   `_homeless` — поле `SoloBase` с задачей, чья бездомная ошибка сейчас
   идёт через хуки. Спека называет только публичные имена, эти выбраны
   здесь.
9. **Zone-value хранит зону вызова как есть, без транзитивного
   разворачивания.** Вложенный `unattended` внутри `unattended` перепишет
   значение зоной внешнего форка, и задача, созданная во внутреннем
   форке, отчитается через внешний обработчик — то есть тому же
   наблюдателю той же задачи. Это верный адрес; городить цепочку «искать
   самый внешний» не за чем.
...
- `lib/src/job_context.dart` — `unattended` в интерфейсе `JobContext` и в
  `JobContextBase`; фильтр отмены; `_UnattendedFork` и `_unattendedKey`;
...
- Даёт дальше: `void unattended(FutureOr<void> Function() action)` в
  `JobContext` и `JobContextBase`; приватные `_UnattendedFork`
  (`({Zone from, JobBase<Object?> owner})`) и `_unattendedKey`
  (`final Object`) на верхнем уровне `job_context.dart` — задачи 2 и 3
  читают их.
...
```dart
  ...
  void unattended(FutureOr<void> Function() action) {
    ...
    runZonedGuarded<void>(
      ...
      _unattendedError,
      zoneValues: {
        _unattendedKey: (from: Zone.current, owner: _owner),
      },
    );
  }
...
```dart
/// What a fork of [JobContext.unattended] carries: the zone the call was
/// made in, and the job that made it.
typedef _UnattendedFork = ({Zone from, JobBase<Object?> owner});

/// The zone-value key of an [_UnattendedFork]. A fresh object, so nothing
/// outside this library can name it.
...
- [ ] **Шаг 6.** Дописать остальные тесты файла. Одиннадцать штук,
  ...
  6. `a body cancelling itself leaves nothing at the observer` — тело
     ...
     `ctx.wait`; строк `error` нет.
  7. `a Cancelled built inside the work is an error like any other` —
     `ctx.unattended(() => throw Cancelled('mine'))`; исход задачи
     `Done`, у наблюдателя строка `error Cancelled(mine)`. Спека,
     «Фильтр отмены», перечень пропускаемого.
  8. `a child cancelled by this job's cascade is not an error, a child
     ...
     есть.
  9. `unattended after the job has finished throws and names itself` —
     контекст, утёкший в замыкание; `expect(() => ctx.unattended(() {}),
     throwsA(isA<StateError>().having((e) => '$e', 'message',
     contains('unattended'))))`.
  10. `a disposer may start unattended work` — `ctx.onDispose(() =>
      ...
      работает».
  11. `unattended on a cancelled but still running job does not throw` —
      задача помечена отменой, тело в `finally` зовёт член; исключения
      нет.
  12. `an abandoned each wrapped in unattended hands a late handler error
...
**Интерфейсы:**
- Берёт из задачи 1: `_UnattendedFork`, `_unattendedKey`.
...
void main() {
  test('run inside unattended work throws and leaves no child behind', () {
    final journal = JobJournal();
    fakeAsync((async) {
      final job = Job<void>(key: 'j', observer: journal, (ctx) async {
        ctx.unattended(() {
          ctx.run(Job.deferred<void>(key: 'child', (c) async {}));
        });
        await ctx.wait(() => delay(1));
      });
      async.flushTimers();
    ...
    expect(
      journal.take().where((line) => line.contains('error')).single,
      contains('cannot run a child inside unattended work'),
    );
  });

  test('uncancellable inside unattended work throws', () {
    final journal = JobJournal();
    fakeAsync((async) {
      Job<void>(key: 'j', observer: journal, (ctx) async {
        ctx.unattended(() => ctx.uncancellable(() => delay(10)));
        await ctx.wait(() => delay(1));
      });
      async.flushTimers();
    });
...
то же правило, что у синхронного броска в задаче 1, тест 4.
...
```dart
  ...
  /// Another job's fork is not this job's business: the check is by
  /// owner.
  @protected
  void throwIfUnattended(String action) {
    final fork = Zone.current[_unattendedKey] as _UnattendedFork?;
    if (fork != null && identical(fork.owner, _owner)) {
...
- [ ] **Шаг 6.** Дописать в файл два теста на границы проверки.
  ...
  2. `a job of its own may run a child from inside another job's fork` —
     ...
     zone-value несёт хозяина, а не голую метку.
...
- [ ] **Шаг 5.** Дописать второй тест: `a job created inside unattended
      work of a child of solo does not report twice`. Стенд —
      `s8_children.dart`. В `packages/solo` его писать нечем без
      контроллера, поэтому здесь проверяется ядерная половина: у задачи,
      созданной в работе, наблюдателя нет, и её провал уходит в зону
      **один раз**, а не и в зону, и наблюдателю хозяина.

- [ ] **Шаг 6.** Прогнать весь пакет.
...
- [ ] **Шаг 7.** Дописать в стенд пять тестов — маршрут закрывается
  ...
  4. `an override without super keeps the error out of the zone` —
     наследник `TestSolo` с пустым `onError`; в зоне пусто, ошибка у
     наследника.
...
`_Watched` — наследник `Solo<TestState>` в самом файле, пишущий в список
из `onFinish`, `onError` и из `close()` после `await super.close()`.
...
Сам `runSolo` не звать: он ставит глобального наблюдателя, а тесты этого
файла смотрят на хуки контроллера.
...
- «Задача, созданная внутри `action`» — задача 3.
...
- «Фильтр отмены» — задача 1, шаг 4 и тесты 5–8; решение по вопросу
  плана — «Решения, принятые в плане сверх спеки», пункт 2.
