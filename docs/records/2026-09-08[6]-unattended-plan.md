> **Состояние на 2026-09-08:** **исполнен целиком и смержен в `main`**,
> семь задач, семь коммитов. Третья редакция, написанная по второму
...
> переписаны механика zone-value и тесты. Отличия исполнения от текста
> плана — в конце записи.
...
Исполнен подряд, задача за задачей, семь коммитов. Дословный код
заработал везде; отличий от текста плана четыре, все мелкие и найденные
прогоном.

1. **Задача 3, шаг 6.** Тест «отчитывается один раз, сам за себя» в
   первом написании падал не на том: провал задачи, созданной в фоне,
   никем не наблюдён и уходит в зону — это и есть проверяемое поведение,
   так что зону надо ловить, а не давать ей уронить тест. Тест ловит её
   хелпером `_inZone` и проверяет троякое: хук чужого контроллера получил
   строку, хук хозяина не получил ничего, в зоне ровно одна запись.
2. **Задача 3.** Импорт `package:jobs/jobs.dart` в
   `packages/solo/test/unattended_test.dart` оказался лишним
   (`unnecessary_import`): всё нужное реэкспортит `package:solo/solo.dart`.
3. **Задача 5.** Три теста оси правил потребовали двух правок под линты,
   которых план не предвидел: `_Recorder` понадобился метод-обёртка над
   защищённым `externalSetState`, а `ctx.emit(...)` и `ctx.unattended(...)`
   подряд дали `cascade_invocations` — тот же линт, что план уже учёл для
   контроллеров, но на другом получателе.
4. **Задача 6, шаг 3.** В дартдоке `each` соседнее слово «unattended» —
   «the handler in flight runs on unattended» — заменено на «unwatched»:
   после появления члена с таким именем оно читалось как ссылка на него.

Проверки в конце: `jobs` 216 тестов, `solo` 266, `packages/solo/example`
7, `flutter_solo` 8; четыре `analyze` чисты, `dart doc` обоих пакетов без
предупреждений, `python3 tool/check_translations.py` — четыре пары, без
расхождений. Точка удара по сьютам нулевая, как и обещано.

Подмены, проверенные по ходу исполнения: снятие фильтра отмены, `check()`
перед форком, `error is! Cancelled` из хука, `Zone.current` вместо
транзитивного `from`. Каждая покрасила ровно тот тест, который под неё
написан.
