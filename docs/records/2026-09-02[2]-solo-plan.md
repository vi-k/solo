> **Состояние на 2026-09-03:** выполнен целиком, задачи 1–20 смержены в
> `main` (`03c8255`..`3e3b299`); правки по итогам финального ревью — в
> следующем коммите. Просмотрен владельцем 2026-09-02 перед исполнением: пол
> SDK поднят до `^3.6.0`, зависимости согласованы с Flutter 3.27, отложенная
> публикация заменена асинхронным стримом.
...
Решения по ходу исполнения, изменившие текст плана. Код и тесты в дереве
главнее этого документа.

- Задачи 3 и 4: тестовая опора собиралась в два приёма, а не в один, как
  клал план. В задаче 3 `TestSolo` — только конструктор (защищённых
  `queue`/`current`/`lastJobWhere`/`externalSetState` у `SoloBase` ещё нет),
  а `runSolo` не закрывает контроллер. Переопределения появились в задаче 4
  вместе с движком, `cancelAll()` в `finally` — там же, `close()` вместо
  него — в задаче 12.
- `_onJobFinished` обнуляет `_current` до `observer.onFinish` и хука
  `onFinish`: хук видит контроллер уже простаивающим и может ставить задачи.
- `ctx.run` регистрирует ребёнка в родителе до веток «родитель отменён» и
  «правила не пускают»: родитель отвечает за каждую задачу, которую пытался
  запустить, даже за отброшенную до старта.
- `Solo.close` (и позже `SoloListenable.close`) запоминает свою футуру в
  `Completer` до вызова `super.close()`: `async`-обёртка отдавала бы новую
  футуру на каждый вызов, а реентерабельный `close` из хука должен получать
  ту же самую.
- `_setState` копит изменения в очереди `_unpublished` и публикует их
  старейшим вперёд: реентерабельный `externalSetState` из хука `onChange`
  иначе клал в стрим новое состояние раньше старого.
- `close` запоминает свой стек-трейс (`_closeStackTrace`), чтобы `add` после
  закрытия отдавал `Cancelled(closed)` со стеком вызова `close`, а не со
  стеком `add`.
- Задача 8: ожидание одного теста поправлено — прокачка очереди планируется
  через `scheduleMicrotask` и успевает отработать в том же `elapse`, поэтому
  `[keep] started` стоит в журнале раньше, чем ожидал план.
- Задача 16: `async.elapsed` после `flushTimers` считает и таймеры брошенных
  ожиданий, поэтому он больше момента последней строки журнала; ожидания по
  времени в тестах исправлены под это.
- Задача 19: тесты `flutter_solo` написаны на настоящем асинхроне
  (`async`-тела, `await Future<void>.microtask(() {})`), а не на `FakeAsync`,
  как велит общее правило тестов. Таймеров в них нет вовсе: `SoloListenable`
  зовёт слушателей синхронно, а стрим отдаёт событие следующей микротаской,
  так что двигать фейковое время нечего.

## Глобальные ограничения

- Пакет `solo`, версия `1.0.0`. Пол SDK во всех трёх пакетах —
  `environment: sdk: ^3.6.0` (правило владельца для всех его проектов,
  2026-09-02). Ничего из языка новее 3.6 не использовать: без `_` как
  wildcard дважды в одном списке параметров (это 3.7).
- Зависимости подняты, но в согласии с Flutter, соответствующим полу SDK
  (решение владельца, 2026-09-02). Это Flutter 3.27.0, первый с Dart 3.6.
  Правило: то, что Flutter 3.27 пинит, берётся с его пином как полом; всё
  остальное — самая свежая версия, чей пол SDK не выше 3.6.0. Итог
  (проверено по pub.dev 2026-09-02):
  рантайм `meta: ^1.15.0` (Flutter 3.27 пинит ровно `1.15.0`);
  dev `fake_async: ^1.3.1` (пин Flutter 3.27), `test: ^1.26.3`
  (`1.27+` требует SDK новее), `lints: ^5.1.1` (`6.x` требует 3.8),
  `flutter_lints: ^5.0.0` (`6.x` требует 3.8). Пакет должен собираться и
  тестироваться на Dart 3.6 / Flutter 3.27, а не только на локальных
  3.13 / 3.47. `clock` и `collection` уходят вместе с кодом 1.x.
...
3. **Стрим асинхронный: `StreamController.broadcast()` без `sync`.**
   Проверено на Dart 3.13.0: у синхронного контроллера `add` из слушателя
   бросает `Bad state: Cannot fire new event. Controller is already firing
   an event`, а спецификация требовала реентерабельный `externalSetState`.
   Владелец 2026-09-02 решил убрать `sync: true` вместо отложенной
   публикации; спецификация поправлена (3.1, 4.8, 4.9, 6.2, 7.3). Само
   состояние `_setState` записывает первым действием, синхронно, до
   observer, хуков и `publish`, поэтому сразу после `ctx.emit(x)` и
   `solo.state`, и `ctx.state` дают `x` (или новее, если хук `onChange`
   успел поставить своё реентерабельно). Слушатели `SoloListenable`
   зовутся синхронно, стрим получает событие следующей микротаской. Тесты
   на это — в задачах 4, 12 и 19.
...
packages/solo/
  ...
  analysis_options.yaml             правится под lints 5.1 в задаче 1b
...
возможности с тестами → 14–16 перенос сценариев 1.x → 17 финальная чистка →
...
- [ ] **Шаг 2.** Зависимости в `pubspec.yaml` привести к целевым (см.
  «Глобальные ограничения»): `clock` убрать, `collection` убрать, остальное
  поднять:
...
- [ ] **Шаг 3.** `dart pub get`, затем `dart analyze`. Код анализировать
  нечего, но `analysis_options.yaml` анализатор читает: правила, о которых
  он говорит «removed»/«deprecated» под `lints` 5.1, убрать;
  `package_api_docs` при удалении заменить на `public_member_api_docs`.
  Команда должна проходить чисто.
...
- [ ] **Шаг 7.** `lib/src/solo.dart` — стрим (решение 3):
...
Строку `[child] removed RemovedByEventRules(is not $Preparing)` (1.x,
строка 4697) закрывают не сценарные тесты этой фазы, а два теста в
`packages/solo/test/children_test.dart`: «a child failing canStart is a
finished handle, parent goes on» и «await value of a child dropped at run
reports the child». Оба показывают её как
`> [child] dropped Cancelled(rules: is not Working)`.
...
### Задача 17. Финальная чистка пакета

**Файлы:**
- Изменить при необходимости: `packages/solo/lib/`, `packages/solo/test/`,
  `packages/solo/analysis_options.yaml`, `packages/solo/pubspec.yaml`

- [ ] **Шаг 1.** `dart pub outdated` из `packages/solo` — ничего, что можно
  поднять в рамках правила о зависимостях; `dart pub get` без
  предупреждений.
- [ ] **Шаг 2.** `dart analyze` — ни одного warning и info. Замечания в
  коде исправить; dartdoc на публичные члены уже есть по задачам 2–12,
...
git commit -m "chore(solo): final cleanup after the 1.x port"
```
...
`focusPoint`, `paused`), с `@immutable` из `package:meta` — поэтому `meta`
стоит в `dependencies` примера.
...
`flutter_lints 5.0.0` — последний с полом SDK не выше 3.6; `6.x` требует
3.8 (см. «Глобальные ограничения»). `analysis_options.yaml` —
...
- [ ] **Шаг 4.** `docs/architecture.md`: в карте модулей отметить
  ...
  3 (стрим асинхронный; реентерабельность только из хуков, observer и
  слушателей `SoloListenable`).
