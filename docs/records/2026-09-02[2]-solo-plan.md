...
3. **Реентерабельность стрима.** Проверено на Dart 3.13.0: у
   ...
   поступления. Стрим остаётся синхронным. Откладывается только доставка в
   стрим: само состояние `_setState` записывает первым действием,
   синхронно, до observer, хуков и `publish`, поэтому сразу после
   `ctx.emit(x)` и `solo.state`, и `ctx.state` уже дают `x` (или ещё более
   новое состояние, если слушатель успел его установить реентерабельно).
   Тест на это — в задаче 12, `expect(solo.state, ...)` внутри тела сразу
   после `emit`.
