...
6. Пункт 6 (метод, а не событие) не вырезан: 2026-09-04 владелец решил
   переименовать его и подрезать — «Methods or a queue, not both», из обеих
   колонок убран третий метод `follow`.
...
цена: вместе с церемонией уходит очередь. Предложение вырезать пункт целиком
отклонено 2026-09-04: пункт назван по тому, что он теперь доказывает, —
«Methods or a queue, not both», — и подрезан, из обеих колонок убран третий
метод `follow`. Кубит больше не «отвечает на пункт 6», а снимает обвязку и
платит за неё очередью; преамбула документа поправлена вслед.
...
подписка на стрим) и предложения вырезать пункт 6: рецепты отложены, а пункт 6
2026-09-04 переименован и подрезан вместо выреза. Вынос раздела сделан целиком,
без трёх пунктов, оставленных в README.
