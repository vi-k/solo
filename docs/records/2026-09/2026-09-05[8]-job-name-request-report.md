> **Состояние на 2026-09-19:** вопрос закрыт. Держатель `job` ответил,
> владелец передал ответ словами «остаёмся на `async_job`»; имя ядра
> на pub.dev — `async_job`, и менять его не нужно. Прежде: решение
> «публиковать как `jobs`» стало неисполнимо — pub.dev отклонил загрузку,
> имя слишком похоже на занятое `job`; разбор отказа — в разделе «Отказ
> pub.dev 2026-09-09» ниже.
