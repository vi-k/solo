...
> плану `2026-09-02[2]-solo-plan.md` (задачи 1–20, коммит на задачу,
> `03c8255`..`3e3b299`); правки по итогам финального ревью — в следующем
> коммите. Пересмотрена 2026-09-03: необслуженный отказ задачи уходит в
> зону, у `Job` появился `ignore()` — правило в новом разделе 5.10, отсылки
> из 5.3 и 5.8. Расхождения реализации со спецификацией перечислены в шапке
> плана. Ранее, 2026-09-02, спецификация была согласована владельцем
> целиком, и в тот же день по его решению стрим стал асинхронным (правки в
> 3.1, 4.8, 4.9, 6.2, 7.3). Дополнена 2026-09-04: у `guard` появился парный
> `ctx.uncancellable(action)` — всё, что здесь сказано про
> `cancellable: false`, читать вместе с
> `2026-09-04[3]-cancellable-design.md`; тогда же изолированы вызовы хуков,
> см. `2026-09-04[2]-hook-isolation-report.md`. Дополнена 2026-09-05:
> сигнатура `emit(S)` ратифицирована явно (раздел 3.7), к семье ожидания
> добавлен `ifCancelled`, а подписка на стрим закрыта extension'ом
> `ctx.each` — обе записи в «Связанных». Пересмотрена 2026-09-12: чтение
> состояния у контроллера зовётся `currentState`, а не `state`, — всё,
> что здесь сказано про `solo.state` и про дубль `value`/`state`, читать
> с этой поправкой; причина — в
> `2026-09-12[3]-current-state-rename-report.md`. Тогда же отменено
> решение «цепочка линейная, а не вилка от базы»: `SoloListenable`
> написан на `SoloBase` и стрима не имеет —
...
> `2026-09-04[3]-cancellable-design.md` (`ctx.uncancellable`),
> `2026-09-04[2]-hook-isolation-report.md` (изоляция хуков),
> `2026-09-05[2]-resource-ownership-design.md` (`ifCancelled`),
> `2026-09-05[3]-stream-subscription-design.md` (`ctx.each`),
> `2026-09-05[1]-generators-migration-review.md` (ревью переезда, находка
> F3 о сигнатуре `emit`).
...
- `emit` доверяем, чтение проверяем. Излучённое состояние не проверяется
  ...
  **Ратифицировано 2026-09-05** по находке F3 обоих ревью переезда
  (`2026-09-05[1]-generators-migration-review.md`): сигнатура остаётся
  `emit(S)`, и это выбор, а не умолчание. Цена названа прямо: у 1.x было
  две защиты — компиляционная (тело было `Stream<W>`, и `yield` принимал
  только `W`) и рантаймовая (новое значение перед публикацией
  перепроверялось), — обе сняты сознательно. Поэтому `W` значит «в каких
  состояниях задача продолжает работу», а не «какие состояния она вправе
  излучить», и от выхода из `W` защищает не тип, а правило «излучай такое
  состояние последней инструкцией тела и больше не трогай `ctx`» (README,
  раздел Concepts). Форма проверена по корпусу: выход из `W` последней
  строкой — нормальная концовка задачи, а не исключение (`vs-bloc`
  пункты 1 и 8, рецепт «Таймаут» и пример захвата ресурса в README).
...
Такая ошибка остаётся делом observer'а и в зону не уходит: исход у задачи
`Cancelled`, а `Cancelled` в зону не отдаётся никогда (5.10).
...
### 5.4a. `onCancel` (добавлено 2026-09-03)

`ctx.onCancel(callback)` регистрирует колбэк, который вызывается в момент
пометки задачи отменённой, до того как об отмене узнает тело. Возвращает
функцию, снимающую регистрацию.

1. Колбэки вызываются в порядке регистрации, синхронно из `_markCancelled`,
   то есть до возобновления тела.
2. Ошибка колбэка уходит в `onError` и там останавливается: ни отмена, ни
   остальные колбэки не страдают.
3. На уже отменённой задаче бросает `Cancelled` (регистрировать нечего и
   начинать нечего), на завершённой — `StateError`.
4. Для задачи, отменённой до старта, колбэков нет: тело не выполнялось.

Зачем: `guard` прекращает ожидание, но не работу. Там, где работа обязана
остановиться по-настоящему — устройство, загрузка, запись, — отмену
передают средствами самой работы: токен отмены, обрыв запроса. Важный
частный случай — железо, которое нельзя вызывать повторно, пока предыдущий
вызов не завершился: тело регистрирует `onCancel`, отдаёт токен устройству и
дожидается возврата вызова, а не бросает его через `guard`. Пока тело ждёт,
задача не считается завершённой, и следующая задача той же очереди не
стартует.
...
Обращение к `done` и к `value` — ещё и отметка о наблюдении исхода (5.10).
...
### 5.10. Необслуженный отказ уходит в зону

Правило (решение владельца от 2026-09-03).

1. Отказ (`Failed`), который никто не наблюдал, движок отдаёт в зону через
   `handleUncaughtError` — ровно как Dart поступает с необработанной
   ошибкой `Future`. Зона берётся та, в которой задача была создана, и
   запоминается в момент создания.
2. Наблюдением считается обращение к `Job.done`, к `Job.value` или вызов
   `Job.ignore()`. Обращение может произойти до завершения задачи или в том
   же микротаске после него: движок даёт одну микротаску отсрочки, как
   Dart.
3. `Cancelled` в зону не уходит никогда.
4. Ошибка, дошедшая до движка только через `onError` (тело бросило после
   отмены; брошенное `guard` действие упало позже), остаётся делом
   observer'а: исход у задачи `Cancelled`, в зону не идёт. Это осознанное
   решение: отменённое действие часто падает по факту отмены, и шуметь этим
   нельзя.
5. Ожидания самого движка наблюдением не являются: ожидание детей
   родителем, `SoloBase.close`, `Job.cancel`. Внутри они ждут завершения
   напрямую, а не через геттер `done`, чтобы движок не выставлял отметку о
   наблюдении за пользователя.

`Job.ignore()` — публичный член интерфейса `Job<T>`, аналог
`Future.ignore()`: говорит движку, что отказ этой задачи никого не
интересует. Для задач, запускаемых по принципу «выстрелил и забыл», чей
отказ уже разобран в `onError`.

Зачем: без этого отказ задачи, которую никто не ждёт и за которой не следит
observer, исчезал без следа. В bloc такая ошибка попадает и в
`BlocObserver.onError`, и в зону; здесь так же, но без наказания тех, кто
отказ обработал.
