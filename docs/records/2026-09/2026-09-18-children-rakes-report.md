...
в документах уже есть — `packages/solo/doc/jobs.md`, метод `stop()`. Проза
следом объясняет, почему метод, который ставит в очередь, изнутри другой задачи
звать нельзя: это ещё одна корневая `Job`.
