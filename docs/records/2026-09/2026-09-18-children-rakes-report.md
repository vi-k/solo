...
завершённая задача со своим местом в дереве: наблюдатель слышит
`> [child] dropped Cancelled(rules: is not Working)` с отступом по уровню,
а `child.done` хранит отмену. В список ожидания такой ребёнок не попадает,
но future от `ctx.run` отмену несёт, и неперехваченной она делает родителя
`Cancelled(handler: …)`. Про наблюдателя в `solo` добавить нечего: там каждая
задача создаётся с наблюдателем контроллера, поэтому из трёх вещей осталось
две.
