...
Дождитесь исхода `Job`, чтобы синхронизировать тест перед проверкой состояния.
Следующий пример использует `package:test`; `FakeProfileApi` является тестовой
реализацией, возвращающей `'Ada Lovelace'`:
...
Для проверки времени и порядка используйте `package:fake_async`. В следующем
тесте фейковое API отвечает через 20 мс. Наблюдатель записывает изменения
состояния и завершения `Job` в один упорядоченный список:
...
`droppable`. Оба вызова метода вернули исходную `Job`. Сбрасывайте глобальный
...
`Future.timeout` ограничивает ожидание future, но саму операцию
не останавливает. Для запроса, результат которого можно отбросить, может
хватить `ctx.wait(() => api.fetch().timeout(...))`.
...
Блок `finally` отменяет таймер при любом выходе. Действительно ли устройство
остановится и какую ошибку вернёт, зависит от его API.
