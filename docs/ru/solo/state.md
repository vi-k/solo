...
`Solo<S>` хранит одно неизменяемое состояние типа `S`. `Job` объявляет,
...
цикл разобран в [Дочерние задачи и стримы](children.md).

Используйте отдельные классы, когда состояния разрешают разные операции,
и общие базовые типы, когда операция может проходить через несколько
состояний.
...
доставки события стрима `currentState` может содержать более новое
значение. Он намеренно назван не `state`: тело задачи видит все члены
своего контроллера без префикса, и чтение без чекпойнта выглядело бы там
в точности как проверяющий `ctx.state`.
...
и контроллер расскажет кому угодно. Здесь это слушатели, вызванные
внутри изменения, как в `SoloListenable`, но на чистом Dart:
...
Слушателя зовут раньше следующей строки того кода, который изменил
состояние, и раньше, чем движок переоценит правила бегущих задач. Новое
состояние он читает сам — ему ничего не передают, поэтому и устареть
прочитанному негде. Он может изменить состояние снова: вложенное
изменение встаёт в очередь публикации, а не обгоняет то, внутри которого
оно случилось. Такое переопределение должно три вещи:
...
Если нужен стрим, он уже есть у `Solo`; это — для случая, когда
микротаска поздно. `SoloListenable` и есть этот код плюс интерфейс
Flutter `ValueListenable`, из-за которого его понимают билдеры и
`Listenable.merge`. В чистом Dart такого интерфейса нет, и своя доставка
говорит только с вашим же кодом — поэтому пакет её и не везёт.

## Внешнее состояние

Независимый источник — устройство, сокет — меняется, не дожидаясь
...
Метод помечен `@protected` и вызывается внутри наследника контроллера,
...
Различие в том, что означает уведомление. Если оно говорит, что
независимая сущность уже изменилась, отражайте этот факт немедленно.
Если оно просит контроллер выполнить работу — обновить данные, сохранить
пришедшее значение, — ставьте обычную `Job`. Доставка события стримом сама
по себе не оправдывает обход очереди.
...
`currentState` и вызывать хуки изменения после `close()`, тогда как
закрытый
...
Держите эти функции в границах вычисления состояния. Освобождение
ресурсов — дело [API уборки](resources.md).
