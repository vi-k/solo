...
и наблюдателям как есть, поэтому строку делает тот слушатель, которому
она нужна. `SoloBase.debug` дополнительно трассирует внутренние операции
очереди и жизненного цикла контроллера.
