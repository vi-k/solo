...
страницы [«Отмена»](cancellation.md) показывает каждый метод ожидания. Отмена
...
и тот, кто её ждёт, сразу получает этот исход, как описывает раздел
[«Отмена работы и закрытие контроллера»](cancellation.md#отмена-работы-и-закрытие-контроллера)
страницы об отмене. `dispose()` поставил `Job` в очередь, `close()` пришёл
