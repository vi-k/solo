...
освобождение устройства. Он использует свою иерархию состояний, отдельную
от примера профиля. `NotDisposed` объединяет все состояния, в которых ещё
...
очистка не даёт применить их к следующей. Работающую `Job` `queue.clear()`
не затрагивает.
