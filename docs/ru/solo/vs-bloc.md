...
`ReadBattery`. Новое событие получает метку текущего поколения — того самого,
которое `Disconnect` только что увеличил, — и проверка метки его пропускает.
Очередь доходит до него уже после отключения, и обработчик зовёт `_ble.battery`
у устройства, которое больше не подключено. Устройство отказывает; bloc
сообщает об ошибке в `onError` и пробрасывает её дальше, а так как обработчика
никто не ждёт, она приходит в зону неперехваченной:
`Bad state: battery: not connected`.

`run<Connected, void>` проверяется на старте `Job`, поэтому то же чтение
заканчивается на `Cancelled(rules: is not Connected)`, и вызова не происходит.
В обработчике такая проверка — ещё один `if` в `switch`, руками, для каждой
команды, которой она нужна.
