...
`SignedOut` можно опубликовать и обычной `Job`. Тогда она встанет в очередь
за сборкой и будет ждать работу, которую сама обесценивает. `externalSetState`
сразу обновляет состояние и проверяет работающие тела по их правилам. Здесь
`run<SignedIn, void>` разрешает сборку только при `SignedIn`, поэтому отзыв
отменяет её с `Cancelled(rules: is not SignedIn)`.

Очередь обходит только то, что уже случилось: сессии больше нет, и ни одна
`Job` в очереди этого не изменит. А если источник просит что-то сделать —
обновить, дозагрузить, повторить, — это обычная работа, и ей место в очереди.
Стрим сам по себе ничего не решает: важно, что пришло, а не откуда.
