...
и `Cubit` на десяти сценариях контроллеров. Каждый раздел описывает нужное
...
извлекает код из этого документа; он находится в репозитории
в `tool/doc_snippets.py`.
...
и в порядке выполнения сдвинулась одна строка:
...
изменения устройства имеют отдельный путь, описанный в разделе 8.
...
индикатором — ровно как незащищённый, — а о сбое сообщают дважды: из `emit`
и из сломанного им обработчика.
...
С `sequential()` прогон показывает, что `close()` ждёт вызов API, который ещё
выполняется, а состояние после него — `ChatState(reply to hi)`: ответ
опубликован в контроллер, который уже закрывался. Последующий `add` затем
бросает `Bad state: Cannot add new events after calling close`, и эта ошибка
приходит в зону, пока `close()` ещё ожидают, а не вызывающему обработчик коду.
...
`concurrent`, `droppable` или `restartable` прогон показывает, что `close()`
возвращается до конца тела, а отменённый emitter игнорирует последующие записи.
Ни один вариант не прерывает вызов API или оставшийся код тела обработчика.
...
работу; `CancelRefresh` отменяет обработчик через ту же регистрацию
с `restartable()`. Пример отслеживает только активность обновления; API
не возвращает данных для отображения.
...
заменить текущий обработчик; отдельная регистрация `on<CancelRefresh>` сама
по себе не отменит `StartRefresh`.
...
обработчики состояния пропускаются; раздел 8 объясняет внешнее состояние,
...
и проверку после await. Каждой команде с другим поведением замены нужна
...
аргументы, а частые обновления перетаскивания не должны оставлять карту
на устаревшей позиции.
...
возвращают `void`. Для возврата результата нужен дополнительный механизм, как
в разделе 7.
...
закрытия остаётся неполной: Cubit не ждёт эту собственную цепочку. В прогоне
с закрытием во время оплаты списание проходит, но последующий `emit(Paid(...))`
...
выдернут, когда публиковался `Calibrated`: событие сбоя ждало своей очереди
...
не соответствует `NotBroken`. Измеренный сценарий сбоя останавливается после
`[0, 1, 2]` с `Cancelled(rules: is not NotBroken)`. Проверка охватывает
и замену, и потерю допустимости состояния.
