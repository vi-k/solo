...
в конце: всякое ожидание внутри тела идёт через контекст. Голый `await` или
`unawaited(...)` тоже выполнится — остановить его нечем, — и то, что он
выполнит, окажется вне отмены, вне рабочего типа и вне `close`. Что этого
нет, не проверяет никто, — то же самое непроверяемое соглашение, за которое
платит bloc-сторона этих пунктов. Разная только цена забывчивости: здесь
вызов выпадает из гарантий, а там пропущенный `isClosed` пишет состояние,
...
Путей выхода при этом больше, чем кажется. Три строки из показанных стоят
там только ради них: `try` вокруг `add` — потому что `add` после `close`
...
приложение закрывается после этого. Секция при этом свою строку
оправдывает: она закрывает окно между началом списания и пометкой, куда
ещё дотянулись бы политика очереди или правило предметной области, пока
задача отказывать не начала.
...
спрашивают: `close()` и `clear(force: true)` убирают его из очереди
начисто — замерено `Cancelled(closed)`, и списания нет. Отсюда и берётся
ветка `Cancelled` у обработчика, вместе с вызовом после `close`, который не
стартует вовсе. У стартовавшей задачи не забирает ничто: `close()`
дожидается списания, и приложение закрывается после него.
