...
`concurrent`, `droppable` или `restartable` вызов `close()` возвращается
...
assert поздняя запись меняет состояние. Это ограничение времени жизни разобрано
...
последовательность тогда включает устаревшее чтение:
...
закрытия остаётся неполной: Cubit не ждёт эту собственную цепочку. При закрытии
во время оплаты списание проходит, но последующий `emit(Paid(...))` бросает
