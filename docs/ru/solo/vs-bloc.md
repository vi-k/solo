...
Они же несут состояние, которое нужно каждой команде. Обновление, сработавшее,
пока экран закрывается, ставит своё чтение за отключением, и это чтение несёт
текущее поколение — проверка метки его пропускает, и обработчик зовёт
`_ble.battery` у только что отпущенного устройства. Устройство отказывает; bloc
...
`Bad state: battery: not connected`. `run<Connected, void>` проверяется
на старте `Job`, поэтому то же чтение заканчивается
на `Cancelled(rules: is not Connected)`, и вызова не происходит. В обработчике
такая проверка — ещё один `if` в `switch`, руками, для каждой команды, которой
она нужна.
