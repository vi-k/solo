...
выполняется после него. `removeWhere` работает только с очередью: уже
начавшегося чтения в ней нет, поэтому тот же вызов не удаляет ничего,
и устройство получает `[connect, battery, disconnect]`.
