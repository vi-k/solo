...
Когда ждущая работа должна уйти, `close(mode: SoloCloseMode.drain)` сначала
проходит очередь, как `sequential()`; его разбирает раздел
[«Отмена работы и закрытие контроллера»](cancellation.md#отмена-работы-и-закрытие-контроллера)
страницы об отмене. `markReplyRead()`, который зовёт каждый ответ, возвращается
там всё равно с `Cancelled(closed)`, потому что закрывающийся контроллер
не берёт новых корневых `Job`.
