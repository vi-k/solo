...
против bloc 9.2.1 с bloc_concurrency 0.3.0 и, в пункте 4, mutex 3.1.0,
фрагменты на `solo` против `solo` 1.0.0, — и все приведённые трассы взяты
из этих запусков.
...
ними, руками: провод уходит под замок — здесь это `Mutex` из
`package:mutex`.
...
Стоит это второго порядка, стоящего рядом с порядком трансформера, и друг
о друге они ничего не знают. `restartable()` считает старый обработчик
заменённым в тот момент, когда пришло событие; провод говорит иначе ещё
один кусок времени, и знает об этом только замок. Строка `emit.isDone` всё
равно нужна: замок упорядочивает записи, но не заканчивает цикл. А под тем
же замком обязан идти каждый вызов устройства, в том числе из
обработчиков, к прошивке отношения не имеющих; никто не проверит, что это
так, и одного пропущенного хватит, чтобы оба образа снова оказались на
проводе — без единой ошибки где бы то ни было.
