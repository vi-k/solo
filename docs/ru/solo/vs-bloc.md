...
final class CheckoutController extends Solo<CheckoutState> {
  ...
  Job<Receipt> pay(Order order) => run<CheckoutState, Receipt>(
        ...
        (ctx) async {
          ctx
            ..emit(const Paying())
            // Списание, ушедшее на сервер, назад не забрать, поэтому
            // отменить задачу нельзя, пока оно не вернётся.
            ..cancellable = false;
          final receipt = await _api.pay(order);
          ctx
            ..cancellable = true
...
`ctx.cancellable` — это всё остальное, и стоит сказать, что делают
...
то есть отмену доложили бы про списанную карту. Дождаться API напрямую —
само по себе не лучше: `close` дождался бы списания, но исход отменённой
задачи — её отмена, и чек всё равно выбросили бы. Закрыть участок — то,
чего списание, уже ушедшее на сервер, и заслуживает: пока закрыто, `cancel`
и `close` получают отказ, а `close` ждёт, поэтому задача возвращается с
`Done(receipt)` про платёж, который действительно состоялся, состояние
заканчивается на `Paid`, и приложение закрывается после этого.
