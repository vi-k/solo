...
`[connect, rename kitchen, disconnect]`, а оба стоящих в очереди чтения
пропускаются. Чего им не выразить — так это экрана, который вернулся раньше,
чем разошлись старые события. Те же пять команд, а за ними `Connect`
открывшегося экрана: он снимает флаг, пока чтения ушедшего экрана ещё стоят
в очереди, поэтому оба снова становятся разрешёнными и выполняются:
`[connect, battery, signal, rename kitchen, disconnect, connect]`. Оба чтения
сделаны для экрана, которого уже нет.
...
Устройство получает `[connect, rename kitchen, disconnect]`. Те же команды
с `Connect` после них оставляют оба чтения за бортом:
...
всё-таки выполняется:
`[connect, battery, rename kitchen, disconnect, connect, battery]`. Сигнал,
отдельный объект, в трассу не попадает. `add` не возвращает результат,
указывающий на пропуск чтения.
