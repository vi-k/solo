...
переставший подходить, отменяет задачу независимо от флага, и этой отмены
секция тоже не держит: замерено, что пометка приходит, пока секция ещё
открыта. И карту ничто из этого не спасает. У `_api.pay` нет токена
отмены, так что к тому времени, когда правило может нарушиться, вызов уже
ушёл; замерено, что секция, голый `await`, `ctx.wait` и `ctx.wait` внутри
секции одинаково кончаются `Cancelled(rules: …)` со списанной картой.
Разница только в том, когда задача перестаёт ждать: через `ctx.wait` она
перестаёт на пометке, и снимок, сделанный там, показывает `charged=false`;
списание всё равно приходит мгновением позже. И до строки `Paid` не
доходит ни один из четырёх: запись есть запись, а отменённая задача писать
не может. У этого платежа такого правила нет, и нарочно: `W` здесь базовый
`CheckoutState`, а `keepWhile` не задан, потому что рабочий тип поуже дал
бы смене состояния отменить платёж в промежутке между списанием и строкой,
которая его записывает. Значит на рецепте, как он показан, секция не меняет
ничего — замерено, что `cancel`, `close` и форсированная чистка дают
`Done(receipt)` и с ней, и без неё, — а нужна она задаче, которая флага не
ставит: она держит те же отмены на одном шаге тела вместо всей задачи.
