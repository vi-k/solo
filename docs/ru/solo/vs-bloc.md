...
Они же несут состояние, которое нужно каждой команде. `run<Connected, void>`
отклоняет чтение батареи, пока устройство не подключено: `Job` заканчивается
на `Cancelled(rules: is not Connected)`, а `_ble.battery` не вызывается.
Обработчик выше проверяет метку, а не состояние, поэтому то же чтение без
подключения за ним доходит до устройства и оставляет
`DeviceState(false, b:100)` — показания батареи для неподключённого устройства.
Защита от этого — ещё один `if` в `switch`, руками, для каждой команды, которой
она нужна.
