...
Первая попытка здесь — та версия, к которой ведёт словарь самого API,
а состояния и трассы под ней — то, что этот код печатает при запуске. Она тоже
написана на bloc: заголовок про bloc после неё добавляет версию, которая
требование выполняет. Там, где одной попытки до этого не хватает, за ней идёт
вторая под своим заголовком. Прочитать попытки до ответа и есть смысл раздела;
тот, кто ловушку уже знает, может сразу перейти к `### Bloc` и `### Solo`.
...
вызов `close()` проходит очередь до конца: работающий обработчик и каждое
событие, ждущее за ним, обрабатываются по очереди, каждый из них ещё может
публиковать состояние, и `close()` возвращается после последнего.
С `concurrent` — тем самым, который регистрация получает по умолчанию, — или
с `restartable` проходить нечего: каждое событие, добавленное до `close()`,
сразу попадает в свой обработчик, потому что его ничто не держит. С `droppable`
события, пришедшие, пока обработчик был занят, отброшены ещё при поступлении.
В этих трёх случаях `close()` возвращается до конца тел, а отменённый emitter
каждого из них игнорирует последующие записи. Ни один вариант не прерывает
вызов API или оставшийся код тела обработчика, и ни один не выбрасывает
событие, которое трансформер уже пропустил.
...
всё ещё ждёт возврата вызова API и обработчика — и каждого сообщения, ждущего
за ними: каждое всё равно уходит в API, пока идёт `close()`, а проверка
отклоняет только его ответ. Отдельная регистрация `MarkReplyRead` безопасна
в этом примере, поскольку не меняет общее состояние; иначе возникло бы
ограничение порядка из раздела 1.
...
поставил бы отдельную корневую `Job` через `run` контроллера; она не является
ребёнком `send`.
...
Сообщение, ждущее за работающим, до API не доходит вовсе: оно кончается
`Cancelled(closed)`, не стартовав, — там, где `sequential()` его бы отправил.
Когда ждущая работа должна уйти,
[`close(mode: SoloCloseMode.drain)`](cancellation.md#отмена-работы-и-закрытие-контроллера)
сначала проходит очередь, как `sequential()`. `markReplyRead()`, который зовёт
каждый ответ, возвращается там всё равно с `Cancelled(closed)`, потому что
закрывающийся контроллер не берёт новых корневых `Job`.
...
обработчик, но не публикует новое состояние, а future, которую вернул вызов
API, продолжает выполняться. Когда она завершается, `finally` вызывает старый,
...
просил пользователь. Что устройство сделало из пяти команд, идущих
одновременно, по состоянию не сказать.
...
выполняется после него. `removeWhere` работает только с очередью: если чтение
уже началось, его она не удаляет. Устройство тогда получает
`[connect, battery, disconnect]`.

У переименования стоит `cancellable: false`, и это вторая половина требования.
Удаление в `disconnect` называет чтение, поэтому переименование переживает его
просто потому, что не названо, — а про вызывающего, который позже сметёт всю
очередь через `cancelAll()` или `queue.clear()`, та строка не говорит ничего.
Переименование, о котором попросил пользователь, обязано дойти до устройства
...
наследника, и вызывающий код строит по одному событию на каждую команду. Здесь
этим словарём служат сами методы.

Они же несут состояние, которое нужно каждой команде. Экран закрывается,
`Disconnect` уже в очереди, и в этот момент таймер обновления добавляет
`ReadBattery`. В версии с метками выше новое событие получает метку текущего
поколения — того самого, которое `Disconnect` только что увеличил, — и проверка
метки его пропускает. Очередь доходит до него уже после отключения,
и обработчик зовёт `_ble.battery` у устройства, которое больше не подключено.
Устройство отказывает; bloc сообщает об ошибке в `onError` и пробрасывает её
дальше, а так как обработчика никто не ждёт, она приходит в зону
неперехваченной: `Bad state: battery: not connected`.

Здесь `run<Connected, void>` проверяется на старте `Job`, поэтому то же чтение
заканчивается на `Cancelled(rules: is not Connected)`, и вызова не происходит.
В bloc тот же запрет пишется руками — ещё один `if` в `switch`, на каждую
команду, которой он нужен.
...
запроса — независимо от того, пошёл ли следом другой заказ.

### Первая попытка

`Bloc.add` возвращает `void`, и этот случай затронут
...
Списание — обычный `await`. `ctx.wait`, `ctx.join` и `ctx.uncancellable` нужны
там, где `Job` можно отменить, и различаются тем, что каждый делает с отменой,
пришедшей во время вызова: `wait` отпускает вызов, `join` остаётся с ним
до ответа, `uncancellable` придерживает отмену до конца шага. Эту `Job`
...
за сборкой, которую оно делает недопустимой. Смотреть здесь стоит на проверку
после `await`. Сработать она тут не может вовсе — единственное событие, которое
изменило бы состояние, стоит в очереди за этим же обработчиком, — поэтому от её
удаления в поведении контроллера не изменится ничего. Экран, следящий за ним,
показывает отчёт сессии, которой больше нет.
...
отчёта. Та же проверка теперь срабатывает: отзыв больше не ждёт за сборкой,
а обработчик сборки и её emitter он при этом не отменяет. Эта проверка — то,
что вторая регистрация делает нужным, а не то, что она заменяет.
...
Если `SignedOut` опубликовать обычной `Job`, она встанет в очередь за сборкой
и будет ждать работу, которую сама обесценивает. `externalSetState` сразу
обновляет состояние и проверяет работающие тела по их правилам. Здесь
`run<SignedIn, void>` разрешает сборку только при `SignedIn`, поэтому отзыв
отменяет её с `Cancelled(rules: is not SignedIn)`.

`externalSetState` — для факта, который уже произошёл: сессии больше нет,
...
запись защищают признаком `isFinished`, а слушатель останавливают в `onClose`,
который приходит, когда все задачи закончились, чтобы `SoloCloseMode.drain`
услышал отзыв, пока крутится его очередь, а отзыв, пришедший после конца, был
отброшен защитой, а не бросил ошибку. Как именно останавливать, решает
приложение; фрагменты показывают только регистрацию.
...
текущую запись и, когда та возвращается, бросает `Cancelled` — до следующей
итерации; заменённая загрузка заканчивается на `Cancelled(manual)`. Замена
начинается после завершения этой `Job`. Наблюдаемая последовательность частей
и отсутствие одновременных записей совпадают со случаем bloc с блокировкой.
...
Оба варианта заканчиваются на `Preview(2)`. Первая попытка освобождает один
буфер из двух — текущий; защищённый вариант освобождает оба, каждый по разу.
