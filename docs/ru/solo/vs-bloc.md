...
class DeviceBloc extends Bloc<DeviceEvent, DeviceState> {
  ...
  DeviceBloc(this._ble) : super(const DeviceState()) {
    ...
    on<DeviceEvent>((e, emit) async {
      switch (e) {
        ...
        case ReadBattery():
          // Чтения нужны были только экрану, который пользователь закрыл.
          if (_leaving) return;
  ...
  void onEvent(DeviceEvent event) {
    // Почему здесь: `add` зовёт это синхронно, поэтому к моменту, когда
    // очередь дойдёт до двух чтений за `Disconnect`, флаг уже выставлен.
    // Сброс тут по той же причине. Внутри `case Connect()` он сработал бы
    // тогда, когда до него дойдёт очередь, сняв флаг перед устаревшими
    // чтениями, которые стоят за ним, и они бы всё равно выполнились.
...
чтения не выполняются вовсе. Цена в том, что правило живёт в поле блока, а
не в том месте вызова, которое знает, что экрана больше нет, и что это
условие, а не удаление: события всё так же идут по очереди и всё так же
доходят до обработчика, а каждой команде, способной устареть, нужна своя
копия этой строки.

Флаг ещё и читается тогда, когда до него дойдёт очередь, а не тогда, когда
был сделан вызов, поэтому обе записи в него должны стоять в `onEvent`.
Перенесите сброс в `case Connect()` — и он снимет флаг перед чтениями,
которые ждут за ним: `[connect, battery, signal, rename kitchen,
disconnect]`, оба устаревших чтения выполнены. Уберите сброс совсем — и
заново открытый экран проглотит собственные чтения: `[connect, disconnect,
connect]`, ни одного чтения заряда. И даже на своём месте флаг ничего не
говорит о том, какие именно события в очереди устарели: откройте экран
заново до того, как очередь опустеет, и чтение, принадлежавшее старому,
тоже выполнится — `[connect, battery, disconnect, connect]`.
