...
закрытие контроллера после её старта. Тело публикует `Paid` до завершения
с квитанцией, а `close()` ждёт этого завершения. Ошибки API по-прежнему дают
`Failed`; состояние ошибки можно задать через `run(onError: ...)`.
...
Поэтому же списание — обычный `await`. `ctx.wait`, `ctx.join`
и `ctx.uncancellable` различаются тем, что каждый делает с отменой, пришедшей
во время вызова: `wait` отпускает вызов, `join` остаётся с ним до ответа,
`uncancellable` придерживает отмену и применяет её, когда шаг кончится, —
защищая шаг, а не исход `Job`, которая идёт дальше. Всем троим нужна отмена,
доходящая до `Job`, а до этой во время работы не доходит ни одна:
`cancellable: false` отклоняет всё отклоняемое, а её правило состояния
принимает любой `CheckoutState`.
