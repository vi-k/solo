...
С `sequential()` измеренное закрытие ждёт вызов API, который ещё выполняется,
а состояние после него — `ChatState(reply to hi)`: ответ опубликован
в контроллер, который уже закрывался. Последующий `add` затем бросает
`Bad state: Cannot add new events after calling close`, и эта ошибка приходит
в зону, пока `close()` ещё ожидают, а не вызывающему обработчик коду.
...
`concurrent`, `droppable` или `restartable` измеренный close возвращается
до конца тела, а отменённый emitter игнорирует последующие записи. Ни один
вариант не прерывает вызов API или оставшийся код тела обработчика.
