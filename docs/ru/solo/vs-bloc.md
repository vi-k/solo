...
Предикат говорит, что уходит, а `cancellable: false` — что уйти не может. Это
разные утверждения: фильтр, называющий чтение, оставляет переименование сегодня
и ничего не говорит про завтрашнего вызывающего, который сметёт очередь
целиком. Переименование, о котором попросил пользователь, обязано дойти
до устройства в обоих случаях, поэтому гарантия принадлежит задаче.
`cancelAll()` и `queue.clear()` такую задачу пропускают, а не удаляют; уносит
её `force: true`.
