...
и ни одна `Job` в очереди этого не изменит. Если же источник просит что-то
сделать — обновить, дозагрузить, повторить, — это обычная работа, и ей место
в очереди. Сам по себе стрим ничего не решает: важно, что пришло, а не откуда.
