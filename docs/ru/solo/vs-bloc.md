...
Состояние ниже — прогресс прошивки, а не последний записанный кусок, и на
обеих сторонах по одной и той же причине. Ни та, ни другая не сообщают про
кусок, который дошёл уже после того, как запуску велели остановиться, —
сообщать некому, — а прогресс запуска, которому нашли замену, уходит вместе
с запуском.
...
Верным это быть перестаёт, когда вызывающий — не экран. «Повтори мой
обычный заказ», сказанное ассистенту, приходит вызовом по платформенному
каналу, и платформа ждёт от обработчика значение: что он вернёт, то
пользователю и скажут, что бросит — то пользователь и услышит, и второго
раза заговорить не будет. Ни `BuildContext`, ни виджета, который можно
перестроить, ни слушателя — функция, которая обязана ответить про
доставшийся ей запрос. `WidgetsBindingObserver.didRequestAppExit`, от
которого фреймворк ждёт `AppExitResponse`, прежде чем отпустить
приложение, и асинхронный гард роутера, от которого роутер ждёт маршрут, —
той же формы: снаружи вашего кода позвонили и держат трубку.
...
— и есть работа контроллера, это пункт 1, поэтому обработчику надо
...
и обработчик после этого — `return (await bloc.pay(order)).orderId`, ровно
то, чего просил пункт: два вызова по одному заказу отвечают одним чеком, а
...
ждать вечно и вызов закончит собственный таймаут платформы; отсев дублей
нельзя поручить `droppable()`, потому что выброшенное событие в обработчик
не заходит и его completer не завершается никогда, — поэтому карта
`_inFlight` написана руками. А выходы, которыми владеет сам bloc, completer
как раз и не видит: после `close()` вызов `pay()` бросает `Bad state:
Cannot add new events after calling close`, и до платформы это доходит
падением, а не ответом, — пункт 7.
...
и обработчик, на котором платформа держит трубку:

```dart
/// За «повтори мой обычный заказ». Что он вернёт, то пользователю и
/// скажут; что бросит — то пользователь и услышит.
...
изменение состояния перед ним, а три исхода — это ровно три ответа, которые
платформа принимает. Поскольку ключ называет заказ, а не метод,
...
у написанной руками карты `_inFlight` выше, только карту писать не надо.
Отменённый и упавший пути приходят исходами, а не футурой, которая никогда
не завершится, а после `close` вызов возвращает задачу, уже завершённую с
`Cancelled(closed)`, — и пользователю говорят, что заказ отменён, вместо
того чтобы обработчик бросил там, где ловить некому.
