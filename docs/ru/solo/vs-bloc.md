...
7, и это одна и та же половина: метод кубита принимает типизированные
...
заказу списывают с карты дважды (7). Остальное он тоже оставляет вам:
управлять нечем — очереди нет (6), трансформеров нет вовсе (1, 4), отмены
нет (9), а `emit` после `close` бросает `Bad state: Cannot emit new states
after calling close` (пункт 3 ниже). Пункт 2 достаётся ему целиком:
наблюдатель работает внутри `BlocBase.emit`, а это один и тот же метод у
обоих. Пункт 10 у него чуть хуже — emitter'а нет, значит нет и
`emit.isDone`, и проверка на устаревание возвращается в собственное поле.
В пунктах 5 и 7 кубит выписан.
...
тела. Все bloc-примеры этого документа стоят в этой форме, а пункты ниже —
