...
`Outcome`, `Policy`, рабочий тип `W` в `run<W, T>`, `ctx.wait` и уборку,
зарегистрированную вместе со значением, `ctx.onCancel`, `SoloObserver` и
очередь.
...
10 у него нет и emitter'а, значит нет и `emit.isDone`, и проверка на
...
закрыта навсегда, а весь контроллер — один `switch`.
...
и об этом пункт 8; здесь ничего не сужается, и читать это можно как
состояние, в котором находится плеер.
...
— и есть работа контроллера, это пункт 6, поэтому вызывающему надо
...
after calling close`, и это пункт 3.
...
приходит. Строка с `Paid` — обычная запись, и она бросила бы отмену вместо
того, чтобы что-то записать; задача закончилась бы `Cancelled(closed)` про
платёж, который действительно состоялся. Перенести запись внутрь секции —
не помощь: исход всё равно определит придержанная отмена.

Обещание держится на `cancellable: false` у задачи: такая задача отказывает
всякой отмене, которой может отказать, с того момента, как стартовала, —
`close` в том числе. С ним задача возвращается с `Done(receipt)`, состояние
заканчивается на `Paid`, и приложение закрывается после этого. Секция при
этом свою строку оправдывает: она закрывает окно между началом списания и
пометкой, куда ещё дотянулись бы политика очереди или правило предметной
области, пока задача отказывать не начала.

Собственные правила задачи до записи тоже не достают — `W` здесь базовый
`CheckoutState`, и `keepWhile` нет. Пара эта не случайна: рабочий тип поуже
дал бы смене состояния отменить платёж в промежутке между списанием и
строкой, которая его записывает.
...
Экран монтажа рисует форму волны выбранного клипа. Декодер выделяет
...
**На bloc.** `restartable()` отменяет emitter старого обработчика, а
...
Публичного места под освобождение у emitter нет: его disposables
...
emitter и которое единственное знает, что буфер вообще есть. Перенесите
