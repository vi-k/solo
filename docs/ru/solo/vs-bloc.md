...
которые излучают состояние. Обвязку из пункта 6 он снимает, а из пункта 5
закрывает половину про ожидание: метод кубита принимает типизированные
аргументы и возвращает значение, которого можно дождаться; felangel
указывает на него в самом
[#1556](https://github.com/felangel/bloc/issues/1556). Остальное он
оставляет вам: управлять нечем — очереди нет (1), трансформеров нет вовсе
(2, 3), отмены нет (4), а `emit` после `close` бросает `Bad state: Cannot
emit new states after calling close` (пункт 7 ниже). В пункте 5 кубит
выписан.
...
**На bloc.** `Cubit` снимает обвязку прямо: кубит и есть методы, — что
другими словами означает: обвязка принадлежит событиям, а не пакету.
