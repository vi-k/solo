...
которые излучают состояние. Пункт 6 он закрывает прямо, а из пункта 5 —
половину про ожидание: метод кубита принимает типизированные аргументы и
возвращает значение, которого можно дождаться; felangel указывает на него
в самом [#1556](https://github.com/felangel/bloc/issues/1556). Остальное
он оставляет вам: управлять нечем — очереди нет (1), трансформеров нет
вовсе (2, 3), отмены нет (4), а `emit` после `close` бросает `Bad state:
Cannot emit new states after calling close` (пункт 7 ниже). В пункте 5
кубит выписан.
...
final class CheckoutController extends Solo<CheckoutState> {
  ...
  Job<Receipt> pay(Order order) => run<CheckoutState, Receipt>(
        ...
        // Списание, ушедшее на сервер, назад не забрать, поэтому задача
        // отменяться отказывается, а `close` её дожидается.
        cancellable: false,
        (ctx) async {
...
`cancellable: false` — это всё остальное, и стоит сказать, что делают
альтернативы. `ctx.guard` прекращает ожидание, но не работу: `close` во
время платежа тут же доложил бы `Cancelled`, а списание прошло бы следом,
то есть отмену доложили бы про списанную карту. Убрать `guard` и дождаться
API напрямую — не лучше: `close` дождался бы списания, но исход отменённой
задачи — её отмена, и чек всё равно выбросили бы. Отказ от отмены — то,
