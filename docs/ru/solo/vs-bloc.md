...
final class CheckoutController extends Solo<CheckoutState> {
  ...
  Job<Receipt> pay(Order order) => run<CheckoutState, Receipt>(
        ...
        // Списание, ушедшее на сервер, назад не забрать. Пока оно не
        // ушло, задачу можно снять, как любую другую.
...
`Cancellable.whileQueued` — это всё остальное, и стоит сказать, что делают
...
задачи — её отмена, и чек всё равно выбросили бы. Объявить задачу
непрерываемой с момента старта — то, чего списание, уже ушедшее на сервер,
и заслуживает: `close` ждёт, задача возвращается с `Done(receipt)` про
платёж, который действительно состоялся, состояние заканчивается на `Paid`,
и приложение закрывается после этого.

Здесь `whileQueued`, а не `never`, потому что два момента — разные. Платёж,
ещё ждущий своей очереди, ничего не отправил, и пользователь вправе
передумать: `job.cancel()` по нему тут же снимает его из очереди, а тот же
вызов по тому, который сейчас списывают, дождётся списания, не останавливая
его. Одно объявление на задаче закрывает оба случая, и вызывающему не надо
знать, в каком из них он находится. Поэтому же ветка `Cancelled` у
