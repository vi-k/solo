...
контроллера. Он наследует `SoloBase<S>` и реализует `ValueListenable<S>`;
стрима у него нет — виджет перестраивается по `value`, а результат операции
ждут через её `Job`. Контроллер профиля сохраняет те же состояния и метод
`load`:
...
Экран может владеть контроллером: создайте его в `initState` и закройте в
`dispose`. Общий контроллер может находиться в существующем контейнере
...
контроллера выполняют обновления через контекст. `ListenableBuilder` и
`AnimatedBuilder` также принимают контроллер, когда самому билдеру значение
состояния не нужно.
