...
отменяет ожидающие группы, а не досылает их. Разделы ниже разбирают
группировку, порядок выполнения и исходы отдельных вызовов.
...
непустым, когда ни одна `Job` не работает. Ожидание времени не занимает
слот выполнения; после старта обработчика очередь ждёт его детей
и уборку перед запуском следующей корневой `Job`.
