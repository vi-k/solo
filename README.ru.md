...
затем выполняется отключение, потому что очередь последовательна, а
отключение добавлено последним. `removeWhere` трогает только очередь: уже
начавшийся `connect` сначала доработает. Очередь — объект, на который можно посмотреть и из которого можно
